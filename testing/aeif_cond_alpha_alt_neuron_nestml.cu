                                      ...
                                      aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{
  ...
  dydx[i_g_exc__d] = (-y[i_g_exc]) / pow(param[i_tau_syn_exc], 2)
                     - 2 * y[i_g_exc__d] / param[i_tau_syn_exc];

  dydx[i_g_inh] = 1.0 * y[i_g_inh__d];

  dydx[i_g_inh__d] = (-y[i_g_inh]) / pow(param[i_tau_syn_inh], 2)
                     - 2 * y[i_g_inh__d] / param[i_tau_syn_inh];

  dydx[i_V_m] = 0;

  dydx[i_w] = param[i_a] * ((-param[i_E_L]) / param[i_tau_w]
                    ...
                    aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{
  ...
  {
    y[i_g_exc__d] += (0.001 * y[N_SCAL_VAR + i_exc_spikes])
                     * (M_E / param[i_tau_syn_exc]) * 1.0 * 1000.0;
    y[N_SCAL_VAR + i_exc_spikes] = 0; // reset the value
  }

  if (y[N_SCAL_VAR + i_inh_spikes] > 0)
  {
    y[i_g_inh__d] += (0.001 * y[N_SCAL_VAR + i_inh_spikes])
                     * (M_E / param[i_tau_syn_inh]) * 1.0 * 1000.0;
    y[N_SCAL_VAR + i_inh_spikes] = 0; // reset the value
  }

  /**
   * Begin NESTML generated code for the onCondition block(s)
   * RK5 path left exactly as in the original generated code.
   **/
...
/*
 * ============================================================
 * POST-INTEGRATION KERNEL (needed only for numeric solver)
 * ============================================================
 *
 * IMPORTANT:
 * This kernel is used only in the experimental odeint/thrust path.
 * The original RK5 path remains unchanged and continues to use
 * ExternalUpdate() exactly as generated.
 */
    ...
    int n_param)
{
  ...
  // Keep same semantics as original generated ExternalUpdate().
                                            ...
                                            int i_group, unsigned long long* seed)
{
  ...
  // Keep original RK5 storage allocation and initialization path in both modes
  rk5_.Init(n_node, n_var_, n_param_, 0.0, h_, rk5_data_struct_);
  var_arr_ = rk5_.GetYArr();
  param_arr_ = rk5_.GetParamArr();

#if USE_ODEINT_THRUST
  // Experimental solver operates on the same raw arrays
...
#if !USE_ODEINT_THRUST
  // Original RK5 path unchanged
  rk5_.Calibrate(time_min, h_, rk5_data_struct_);
#else
  // Numeric solver does not use RK5 calibration, but keep h-values consistent
  (void)time_min;
#endif
...
{
#if USE_ODEINT_THRUST
  // Experimental numeric integration
  float dt = 0.0f;
  ...
  // Apply events after numeric integration
