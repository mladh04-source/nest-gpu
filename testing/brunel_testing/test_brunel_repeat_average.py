...
def plot_summary(summary_rows, outdir):
    ...
    building = [r["building_ms_per_neuron_mean"] for r in summary_rows]
    building_err = [r["building_ms_per_neuron_std"] for r in summary_rows]

    simulation = [r["simulation_ms_per_neuron_mean"] for r in summary_rows]
    simulation_err = [r["simulation_ms_per_neuron_std"] for r in summary_rows]

    exc_norm = [r["exc_rate_hz_per_neuron_count_mean"] for r in summary_rows]
    exc_norm_err = [r["exc_rate_hz_per_neuron_count_std"] for r in summary_rows]

    inh_norm = [r["inh_rate_hz_per_neuron_count_mean"] for r in summary_rows]
    inh_norm_err = [r["inh_rate_hz_per_neuron_count_std"] for r in summary_rows]

    fig_width = max(14, len(summary_rows) * 0.9)
    fig_height = 7
    ...
    plt.bar(
        ...
        yerr=inh_norm_err,
        capsize=3,
        label="inh rate / neuron count",
    )

    plt.ylabel("normalized value")
    plt.title("Repeated Brunel test: normalized mean values over 5 runs")
    plt.xticks(x, labels, rotation=45, ha="right")
    plt.legend()
    plt.grid(axis="y", alpha=0.3)
    plt.tight_layout()

    plot_file = outdir / "brunel_repeat_average_normalized_summary.png"
...
def print_summary_table(summary_rows):
    ...
    for r in summary_rows:
        ...
        print(
            ...
            f"{r['exc_rate_hz_per_neuron_count_mean']:>14.8f} "
            f"{r['inh_rate_hz_per_neuron_count_mean']:>14.8f} "
            f"{cv_str:>10}"
        )
