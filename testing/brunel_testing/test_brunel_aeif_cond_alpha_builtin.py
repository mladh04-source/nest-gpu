...
def raster_plot(
    ...
    e_times = (
        np.array([x[1] for x in e_st])
        if len(e_st)
        else np.array([])
    )

    i_ids = (
        np.array([x[0] for x in i_st])
        if len(i_st)
        else np.array([])
    )

    i_times = (
        np.array([x[1] for x in i_st])
        if len(i_st)
        else np.array([])
    )
...
def compute_stats(
    ...
    rate_ex = (
        ecount / (NE * sim_time) * 1000.0
        if NE > 0
        else 0.0
    )

    rate_in = (
        icount / (NI * sim_time) * 1000.0
        if NI > 0
        else 0.0
    )
...
def main():
    ...
    available_params = ngpu.GetScalParamNames(
        int(neuron[0])
    )
