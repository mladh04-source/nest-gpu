...
def create_iaf_model(ngpu, impl, cfg):
    ...
    elif impl == "old_nestml":
        # The old generated NESTML implementation is loaded through
        # nest-gpu_old_nestml/install_old_nestml/bin/nestgpu_vars.sh.
        # The model name itself stays the generated NESTML model name.
...
def create_aeif_model(ngpu, impl, cfg):
    ...
    if impl == "builtin":
        ...
        # Same model name as generated NESTML code, but loaded from old_nestml install.
...
def connect_brunel_network(ngpu, family, impl, neuron, cfg):
    ...
    if family == "iaf":
        if impl == "builtin":
            ...
            # Old NESTML uses the generated NESTML sign convention.
            w_in = g * w_ex
            poiss_weight = 97.0
...
def print_summary(outdir, family):
    ...
    old_nestml = load_stats(outdir, family, "old_nestml")
...
def main():
    ...
    parser.add_argument("--impl", choices=["builtin", "old_nestml"], default=None)
    parser.add_argument("--n-neurons", type=int, default=5000)
    parser.add_argument("--sim-time", type=float, default=1000.0)
    parser.add_argument("--outdir", default="brunel_compare_old_nestml_out")

    args = parser.parse_args()

    if args.worker:
        if args.impl is None:
            raise RuntimeError("--worker requires --impl builtin or --impl old_nestml.")
        run_worker(args)
    else:
        run_parent(args)
