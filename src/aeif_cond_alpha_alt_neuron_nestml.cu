                                            ...
                                            int i_group, unsigned long long* seed)
{
  ...
  /*
   * New:
   * initialize odeint/thrust backend instead of handwritten rk5 backend.
   * Information for me:
   * The wrapper should allocate and initialize x/y/param arrays using the
   * same NodeInit logic as before, so the model code stays as unchanged as possible.
   */
  odeint_solver_ =
    new AeifCondAlphaAltOdeintSolver<N_SCAL_VAR + N_PORT_VAR, N_SCAL_PARAM>(
      n_node, n_var_, n_param_, 0.0, h_, rk5_data_struct_);

  var_arr_ = odeint_solver_->GetYArr();
  param_arr_ = odeint_solver_->GetParamArr();
...
int aeif_cond_alpha_alt_neuron_nestml::Calibrate(double time_min, float time_resolution)
{
  ...
  /*
   * New:
   * pass calibration info to odeint/thrust wrapper.
   */
  odeint_solver_->Calibrate(time_min, h_);
...
int aeif_cond_alpha_alt_neuron_nestml::Update(long long it, double t1)
{
  /*
   * New:
   * one global step through Boost.odeint + Thrust backend.
   */
  odeint_solver_->Step(t1, h_min_, rk5_data_struct_);
  return 0;
}
