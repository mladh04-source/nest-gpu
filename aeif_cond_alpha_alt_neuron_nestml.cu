	      ...
	      aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{
  ...
  param[i_tau_syn_exc] = 0.2; // as ms


  param[i_tau_syn_inh] = 2.0; // as ms


  param[i_E_exc] = 0; // as mV


  param[i_E_inh] = (-85.0); // as mV
  ...
  y[i_g_exc__d] = 0; // as nS / ms


  y[i_g_inh] = 0; // as nS


  y[i_g_inh__d] = 0; // as nS / ms
...
__device__
    void Derivatives_g_exc_g_inh_refr_t_w(double x, float *y, float *dydx, float *param,
		     aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{  
...
__device__
    void Derivatives_V_m_g_exc_g_inh_w(double x, float *y, float *dydx, float *param,
		     aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{  
    ...
    dydx[i_V_m] =(param[i_Delta_T] * param[i_g_L] * exp(((-param[i_V_th]) + min(y[i_V_m], param[i_V_peak])) / param[i_Delta_T]) + param[i_E_L] * param[i_g_L] + param[i_E_exc] * y[i_g_exc] + param[i_E_inh] * y[i_g_inh] + param[i_I_e] + param[i_I_stim] - param[i_g_L] * min(y[i_V_m], param[i_V_peak]) - y[i_g_exc] * min(y[i_V_m], param[i_V_peak]) - y[i_g_inh] * min(y[i_V_m], param[i_V_peak]) - y[i_w]) / param[i_C_m];
			...
			aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{
  ...
  {  
    ...
    **/
    ...
    {      
      y[i_g_exc__d] += (0.001 * y[N_SCAL_VAR + i_exc_spikes]) * (M_E / param[i_tau_syn_exc]) * 1.0 * 1000.0;      
      y[N_SCAL_VAR + i_exc_spikes] = 0; // reset the value
    }
    if (y[N_SCAL_VAR + i_inh_spikes] > 0)
    {      
      y[i_g_inh__d] += (0.001 * y[N_SCAL_VAR + i_inh_spikes]) * (M_E / param[i_tau_syn_inh]) * 1.0 * 1000.0;      
...
void NodeInit(int n_var, int n_param, double x, float *y,
	     float *param, aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{
   aeif_cond_alpha_alt_neuron_nestml_ns::NodeInit(n_var, n_param, x, y, param, data_struct);
}
...
{
    aeif_cond_alpha_alt_neuron_nestml_ns::NodeCalibrate(n_var, n_param, x, y, param, data_struct);
}
...
{
/* we have now problem!
ExternalUpdate cannot be correctly integrated.
the problem is our stepper_.do_step(...) in odeint_thrust_solver.cu in src.

This also means there is no end_time_step.

There is no way to call ExternalUpdate from within the integrator. 
it's only globally, after the step has completed.

the reason:
In odeint, there is no neuron-local callback.
Only observer(state, t) → host-side.
*/
    aeif_cond_alpha_alt_neuron_nestml_ns::ExternalUpdate<NVAR, NPARAM>(x, y, param,
						    end_time_step,
						    data_struct);
}
		 ...
		 aeif_cond_alpha_alt_neuron_nestml_rk5 data_struct)
{
    if(y[i_refr_t] > 0)
    {
      aeif_cond_alpha_alt_neuron_nestml_ns::Derivatives_g_exc_g_inh_refr_t_w<NVAR, NPARAM>(x, y, dydx, param, data_struct);
    }
    else
    {
      aeif_cond_alpha_alt_neuron_nestml_ns::Derivatives_V_m_g_exc_g_inh_w<NVAR, NPARAM>(x, y, dydx, param, data_struct);
    }
}
			 ...
			 int i_group)
{
  ...
  group_param_name_ = aeif_cond_alpha_alt_neuron_nestml_group_param_name;
  ...
  // ---------- Reference solver (correct NEST GPU behavior) ----------
  rk5_.Init(n_node, n_var_, n_param_, 0.0, h_, rk5_data_struct_);
  
#if USE_ODEINT_THRUST
  // ================================================================
  // EXPERIMENTAL: Boost.odeint + Thrust
  // This intentionally violates NEST GPU assumptions.
  // ================================================================
// odeint_solver_ is acceptable for the experimental comparison,
// but would require proper cleanup in production code. 
  odeint_solver_ =
      new OdeintThrustSolver<N_SCAL_VAR + N_PORT_VAR>(n_node);

  // PROBLEM:
  // var_arr_ now points to a Thrust-managed device vector.
  // NEST GPU assumes raw CUDA arrays updated by kernels.
  // NodeInit() is never called for odeint states.
  // This violates NESTML/NEST GPU initialization semantics.
  var_arr_ = odeint_solver_->GetYRaw();
  // IMPORTANT:
  // Parameters are still stored in RK5 memory.
  // odeint does NOT manage parameters.
...
// I changed only this if statement    
#if !USE_ODEINT_THRUST  
...
// few changes
#if USE_ODEINT_THRUST
  // ----------------------------------------------------
  // odeint uses ONE global time step for ALL neurons
  // There is no neuron-local adaptive stepping.
  // ExternalUpdate cannot be called from inside the integrator.
  // ----------------------------------------------------
  float dt = h_;
  ...
  // PROBLEM:
  // ExternalUpdate() (spikes, reset, threshold)
  // cannot be invoked neuron-locally here.
#else
  rk5_.Update<N_SCAL_VAR + N_PORT_VAR, N_SCAL_PARAM>(
      t1, h_min_, rk5_data_struct_);
#endif
