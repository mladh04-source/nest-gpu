...
{  
    ...
    dydx[i_V_m] =param[i_g_L] * (param[i_E_L] / param[i_C_m] - min(y[i_V_m], param[i_V_peak]) / param[i_C_m]) + param[i_Delta_T] * param[i_g_L] * exp(((-param[i_V_th]) + min(y[i_V_m], param[i_V_peak])) / param[i_Delta_T]) / param[i_C_m] + param[i_I_e] / param[i_C_m] + param[i_I_stim] / param[i_C_m] + (y[i_I_syn_exc] - y[i_I_syn_inh] - y[i_w]) / param[i_C_m];
